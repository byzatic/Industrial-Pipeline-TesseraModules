package io.github.byzatic.tessera.industrial_pipeline.workflowroutine.processing_status.processor;
...
public class Processor implements ProcessorInterface {
    ...
    public void process(String commandLineInput)
        ...
        try {
            String processedCommandLineInput =
                    processTemplate(commandLineInput);
    ...
    String processTemplate(String input)
            throws MCg3ApiOperationIncompleteException {
        ...
        while (marker >= 0) {
            ...
            result.append(input, position, marker);

            int expressionEnd =
                    findExpressionEnd(input, marker + 2);
    ...
    private void appendExpression(
        ...
        if (position < end && source.charAt(position) != ')') {
            ...
            ParseResult parsedArgument =
                    parseArgument(source, position, end);

            argument = parsedArgument.value();
            position = parsedArgument.nextPosition();
     ...
     * Эквивалент:
     *
     * (graphPath + "." + nodeName).hashCode()
     *
     * но без создания промежуточной String.
     */
