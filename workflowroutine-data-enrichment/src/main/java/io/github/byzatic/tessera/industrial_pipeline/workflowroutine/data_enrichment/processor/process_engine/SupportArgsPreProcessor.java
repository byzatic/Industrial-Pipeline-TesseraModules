package io.github.byzatic.tessera.industrial_pipeline.workflowroutine.data_enrichment.processor.process_engine;
...
public class SupportArgsPreProcessor {
    ...
    public static List<Argument> searchArgs(List<String> args, String argName) {
        ...
        Map<String, String> stringStringMap = extract(args);
        for (Map.Entry<String, String> argumentEntry : stringStringMap.entrySet()) {
            if (argumentEntry.getKey().equals(argName)) {
                argumentList.add(Argument.newBuilder()
                        .setKey(String.copyValueOf(argumentEntry.getKey().toCharArray()))
                        .setValue(String.copyValueOf(argumentEntry.getValue().toCharArray()))
                        .build());
            }
        }
